...
echo "hammer host set-parameter --host-id  $HOST_ID --name bonds --value '( [bond0]=\"onboot none\" [bond0.140]=\"onboot static vlan ${API_IP}/${API_NM} [bond0.202]=\"onboot static vlan ${NOVA_PRIVATE_IP}/${NOVA_PRIVATE_NM} promisc\" [bond0.170]=\"onboot static vlan ${STORAGE_IP}/${STORAGE_NM} \" [bond1]=\"onboot static ${NOVA_PUBLIC_IP}/${NOVA_PUBLIC_NM} )'"
#hammer host set-parameter --host-id  $HOST_ID --name bonds --value '( [bond0]="onboot none" [bond0.140]="onboot static vlan \"${API_IP}\"/\"${API_NM}\" [bond0.202]="onboot static vlan \"${NOVA_PRIVATE_IP}\"/\"${NOVA_PRIVATE_NM}\" promisc" [bond0.170]="onboot static vlan \"${STORAGE_IP}\"/\"${STORAGE_NM}\" " [bond1]="onboot static \"${NOVA_PUBLIC_IP}\"/\"${NOVA_PUBLIC_NM}\" )'
