...
class Networks(object):
  ...
  def parse_args(self):
    ...
    parser.add_argument("-n", "--node_name", default=socket.gethostname(), help="name of this node in the network json config file", action="store")
  ...
  def validate_network_communication(self):
    ...
    # Allow the user to specify a shortened node name
    node_name = self.args.node_name
    if not (node_name in self.network_config):
      node_name = "overcloud-" + node_name + ".localdomain"

    networks = self.network_config[node_name]["networks"]
    for network in networks.keys():
      logger.info(" {0} pinging network {1}: ".format(node_name, network))
