...
# Confirm that nova-evacuate is started after the floating IP resources,
...
def confirm_nova_evacuate_resource(first_controller_node_ip):
    ...
    for ip in IPS:
        ssh_cmd(first_controller_node_ip, "heat-admin",
                "sudo pcs constraint order start " + ip +
                " then nova-evacuate")

    resource_list = ['openstack-glance-api-clone',
...
# the controllers identified above,
...
# Populate the Compute node resources within pacemaker, starting with
...
def populate_compute_nodes_resources(first_controller_node_ip):
    ...
    ssh_cmd(first_controller_node_ip, "heat-admin",
            ...
            " ceilometer-compute systemd:openstack-ceilometer-compute" +
            " --clone interleave=true --disabled --force")

    ssh_cmd(first_controller_node_ip, "heat-admin",
            "sudo pcs constraint location ceilometer-compute-clone" +
    ...
    ssh_cmd(first_controller_node_ip, "heat-admin",
            ...
            " then ceilometer-compute-clone")

    ssh_cmd(first_controller_node_ip, "heat-admin",
            "sudo pcs constraint colocation add ceilometer-compute-clone" +
    ...
    ssh_cmd(first_controller_node_ip, "heat-admin",
            ...
            " ocf:openstack:nova-compute-wait auth_url=" +
            oc_auth_url + " username=" +
            oc_username + " password=" +
            oc_password + " tenant_name=" +
            oc_tenant_name +
    ...
    ssh_cmd(first_controller_node_ip, "heat-admin",
            "sudo pcs constraint order start openstack-nova-conductor-clone" +
            " then nova-compute-checkevacuate-clone require-all=false")

    ssh_cmd(first_controller_node_ip, "heat-admin",
            "sudo pcs resource create nova-compute" +
            " systemd:openstack-nova-compute --clone" +
...
def main():
    ...
    if args.compute_node_ip == '' and args.controller_node_ip == '':
        ...
        distribute_all_authkey(compute_nodes_ip,
                               controller_nodes_ip)
