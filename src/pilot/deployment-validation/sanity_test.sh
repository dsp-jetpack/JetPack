...
STORAGE_NETWORK=$(get_value storage_network)
STORAGE_NETWORK_START_IP=$(get_value storage_network_start_ip)
STORAGE_NETWORK_END_IP=$(get_value storage_network_end_ip)
STORAGE_NETWORK_GATEWAY=$(get_value storage_network_gateway)
STORAGE_NETWORK_VLAN=$(get_value storage_network_vlan)
STORAGE_NETWORK_NAME=$(get_value storage_network_name)
STORAGE_SUBNET_NAME=$(get_value storage_subnet_name)
...
setup_manila(){
  ...
  set_admin_scope
  ext_net_exists=$(openstack network list -c Name -f value | grep "$STORAGE_NETWORK_NAME")
  if [ "$ext_net_exists" != "$STORAGE_NETWORK_NAME" ]
  then
    execute_command "openstack network create $STORAGE_NETWORK_NAME --external --provider-network-type vlan --provider-physical-network physext --provider-segment $STORAGE_NETWORK_VLAN"
    execute_command "openstack subnet create $STORAGE_SUBNET_NAME --network $STORAGE_NETWORK_NAME --subnet-range $STORAGE_NETWORK --allocation-pool start=$STORAGE_NETWORK_START_IP,end=$STORAGE_NETWORK_END_IP --gateway $STORAGE_NETWORK_GATEWAY --no-dhcp"
  else
    info "#----- External network '$STORAGE_NETWORK_NAME' exists. Skipping"
  fi
  ...
  if [ "$manila_share_network_exists" != "unity_share_net" ]; then
    net_id=$(openstack network list | grep " $STORAGE_NETWORK_NAME " | head -n 1 | awk '{print $2}')
    subnet_id=$(openstack network list | grep $STORAGE_NETWORK_NAME | head -n 1 | awk '{print $6}')
...
then
  ...
  network_ids=$(openstack network list | grep -E "$BASE_TENANT_NETWORK_NAME|$VLAN_NETWORK_NAME|$FLOATING_IP_NETWORK_NAME|$STORAGE_NETWORK_NAME" | awk '{print $2}')
