...
class Profile():
    ...
    def __init__(self):
        ...
        if not os.path.isfile(self.definition):
            raise AssertionError(self.profile_name + " is not a valid profile in your .ini settings")
    ...
    def validate_configuration(self):
        ...
        profile_config.read(os.path.dirname(inspect.getfile(Settings)) + "/" + profile_definition['sample_ini'])
        Err = ''
                    ...
                    for set, vals  in value.iteritems():
                               ...
                               for test in vals['validate']:
                                   if 'should_be_valid_ip' in test :
                                       if  self.is_valid_ip(user_config.get(str(stanza), set)) is False:
                                           Err = Err + "\nSetting for " + set + " Should be a valid ip adress\n"
                               validated = True
                        except:
                            pass
                        ...
                        if len(allowed_settings) > 0 :
                           if user_config.get(str(stanza), set) in allowed_settings:
                               ...
                               Err = Err  + "\nYour setting for " + stanza + "::" + set + " is not valid for the profile " + self.profile_name + \
                                     " it should be to one of the following options : " +json.dumps(allowed_settings)
                        elif validated is True:
                            pass
                        else:
                            if profile_config.get(str(stanza), set) != user_config.get(str(stanza), set):
                                 Err = Err + "\nYour setting for " + stanza + "::" + set + " is not valid for the profile " + self.profile_name + \
                                    " it should be to " + profile_config.get(str(stanza), set)
                        
            
        if len(Err) > 0:
            logger.info(Err)
            msg = "Your settings diverge from the " + self.profile_name + " profile" + \
                  "\nReview the settings and errors above in your .ini file " + \
