...
class Checkpoints():
    ...
    def verify_subscription_status(external_ip, user, password, retries):
        i = 0
        subscription_status = Ssh.execute_command(external_ip, user, password, "subscription-manager status")[0]
    ...
    def verify_pools_attached(ip_addr, user, password, logfile):
        ...
        log_out = Ssh.execute_command(ip_addr, user, password, "cat " + logfile)[0]
        error1 = 'No subscriptions are available from the pool with'
        error2 = 'Removed temporarly as this error will show when not pulling from the cdn but internal repos'
    ...
    def ping_host(self, external_ip, user, passwd, target_host):
        for i in range(1, 30):
            ping_status = Ssh.execute_command(external_ip, user, passwd, "ping " + target_host + " -c 1 -w 30 ")[0]
            if self.ping_success in ping_status:
                logger.debug("Ping {} successful on attempt #{}".format(target_host, i))
                break
    ...
    def sah_health_check(self):
        ...
        subscription_status = self.verify_subscription_status(self.settings.sah_node.external_ip, "root",
                                                              self.settings.sah_node.root_password,
                                                              self.settings.subscription_check_retries)
        if "Current" not in subscription_status:
            raise AssertionError("SAH did not register properly : " + subscription_status)

        logger.debug("*** Verify the SAH can ping its public gateway")
        test = self.ping_host(self.settings.sah_node.external_ip, "root", self.settings.sah_node.root_password,
        ...
        if "No such file" in \
            ...
            raise AssertionError(
                "KVM Not running on the SAH node - make sure the node has been "
                "DTK'ed/Virtualization enabled in the Bios")
    ...
    def director_vm_health_check(self):
        ...
        subscription_status = self.verify_subscription_status(self.settings.director_node.external_ip, "root",
                                                              self.settings.director_node.root_password,
                                                              self.settings.subscription_check_retries)
        if "Current" not in subscription_status:
            raise AssertionError("Director VM did not register properly : " + subscription_status)

        logger.debug("*** Verify all pools registered & repositories subscribed ***")
        if self.verify_pools_attached(self.settings.director_node.external_ip, "root",
                                      self.settings.director_node.root_password,
                                      "/root/" + self.settings.director_node.hostname + "-posts.log") is False:
        ...
        test = self.ping_host(self.settings.director_node.external_ip, "root",
                              self.settings.director_node.root_password, self.settings.external_gateway)
        if self.ping_success not in test:
        ...
        test = self.ping_host(self.settings.director_node.external_ip, "root",
                              self.settings.director_node.root_password, "8.8.8.8")
        if self.ping_success not in test:
        ...
        test = self.ping_host(self.settings.director_node.external_ip, "root",
                              self.settings.director_node.root_password, "google.com")
        if self.ping_success not in test:
        ...
        test = self.ping_host(self.settings.director_node.external_ip, "root",
                              self.settings.director_node.root_password, self.settings.sah_node.provisioning_ip)
        if self.ping_success not in test:
        ...
        test = self.ping_host(self.settings.director_node.external_ip, "root",
                              self.settings.director_node.root_password, self.settings.sah_node.external_ip)
        if self.ping_success not in test:
            raise AssertionError("Director VM cannot ping the SAH node through the provisioning network : " + test)

        logger.debug("*** Verify the Director VM can ping the idrac network")
        test = self.ping_host(self.settings.director_node.external_ip, "root",
                              self.settings.director_node.root_password, self.settings.controller_nodes[0].idrac_ip)
        if self.ping_success not in test:
    ...
    def ceph_vm_health_check(self):
        ...
        subscription_status = self.verify_subscription_status(self.settings.ceph_node.external_ip, "root",
                                                              self.settings.ceph_node.root_password,
                                                              self.settings.subscription_check_retries)
        if "Current" not in subscription_status:
            raise AssertionError("Ceph VM did not register properly : " + subscription_status)

        logger.debug("*** Verify the Ceph VM can ping its public gateway")
        test = self.ping_host(self.settings.ceph_node.external_ip, "root", self.settings.ceph_node.root_password,
    ...
    def verify_nodes_registered_in_ironic(self):
        ...
        ls_nodes = re[0].split("\n")
        ls_nodes.pop()
        expected_nodes = len(self.settings.controller_nodes) + len(self.settings.compute_nodes) + len(
            self.settings.ceph_nodes)
        if len(ls_nodes) != expected_nodes:
            raise AssertionError(
                "Expected amount of nodes registered in Ironic does not add up " + str(len(list)) + "/" + str(
                    expected_nodes))
    ...
    def verify_undercloud_installed(self):
        ...
        if "No such file or directory" in re[0]:
            raise AssertionError(
                "Director & Undercloud did not install properly, check /pilot/install-director.log for details")
    ...
    def verify_computes_virtualization_enabled(self):
        ...
        for each in computes:
            ...
            if "No such file" in re[0]:
                raise AssertionError(
                    "KVM Not running on Compute node '{}' - make sure the node "
                    "has been DTK'ed/Virtualization enabled in the Bios".format(
                        provisioning_ip))
    ...
    def retreive_switches_config(self):
        ...
        for each in self.settings.switches:
            ...
            logger.info(self.execute_as_shell(each.ip, each.user, each.password,
                                              'copy running-config scp://' + self.settings.bastion_host_user + ':'
                                              + self.settings.bastion_host_password
                                              + '@' + self.settings.bastion_host_ip + '//auto_results/switch-config-'
                                              + each.switch_name))
