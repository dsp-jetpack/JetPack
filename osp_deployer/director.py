...
class Director():
    def __init__(self):
        ...
        logger.debug(
            Ssh.execute_command(self.settings.director_node.external_ip, self.settings.director_install_account_user,
    ...
    def apply_internal_repos(self):
        ...
        if self.settings.internal_repos is True:
            ...
            for repo in self.settings.internal_repos_urls:
                ...
                logger.debug(Ssh.execute_command(self.settings.director_node.external_ip, "root",
                                                 self.settings.director_node.root_password, cmd))
                logger.debug(Ssh.execute_command(self.settings.director_node.external_ip, "root",
                                                 self.settings.director_node.root_password,
                                                 "sed -i '/enabled=1/a priority=1' /etc/yum.repos.d/internal_" + str(
                                                     count) + ".repo"))
                count += 1
        ...
        cmd = "sudo sed -i \"s/.*Keystone_domain\['heat_domain'\].*/Service\['keystone'\] -> Class\['::keystone::roles::admin'\]" \
              " -> Class\['::heat::keystone::domain'\]/\" " \
              "/usr/share/instack-undercloud/puppet-stack-config/puppet-stack-config.pp"
        logger.debug(Ssh.execute_command_tty(self.settings.director_node.external_ip, "root",
                                             self.settings.director_node.root_password, cmd))
    ...
    def upload_update_conf_files(self):
        ...
        Scp.put_file(self.settings.director_node.external_ip, install_admin_user, install_admin_password,
                     "/root/pilot.tar.gz", remote_file)

        cmd = 'cd;tar zxvf pilot.tar.gz -C pilot'
        logger.debug(
            Ssh.execute_command(self.settings.director_node.external_ip, install_admin_user, install_admin_password,
                                cmd))

        cmds = [
            'sed -i "s|local_ip = .*|local_ip = ' + self.settings.director_node.provisioning_ip
            + '/24|" pilot/undercloud.conf',
            'sed -i "s|local_interface = .*|local_interface = eth1|" pilot/undercloud.conf',
            'sed -i "s|masquerade_network = .*|masquerade_network = ' + self.settings.provisioning_network
            + '|" pilot/undercloud.conf',
            'sed -i "s|dhcp_start = .*|dhcp_start = ' + self.settings.provisioning_net_dhcp_start
            + '|" pilot/undercloud.conf',
            'sed -i "s|dhcp_end = .*|dhcp_end = ' + self.settings.provisioning_net_dhcp_end
            + '|" pilot/undercloud.conf',
            'sed -i "s|network_cidr = .*|network_cidr = ' + self.settings.provisioning_network
            + '|" pilot/undercloud.conf',
            'sed -i "s|network_gateway = .*|network_gateway = ' + self.settings.director_node.provisioning_ip
            + '|" pilot/undercloud.conf',
            'sed -i "s|inspection_iprange = .*|inspection_iprange = ' + self.settings.discovery_ip_range
            + '|" pilot/undercloud.conf',
        ]
        for cmd in cmds:
            logger.debug(
    ...
    def install_director(self):
        ...
        Ssh.execute_command_tty(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                                self.settings.director_install_account_pwd, cmd)
    ...
    def upload_cloud_images(self):
        ...
        remote_file = "/home/" + self.settings.director_install_account_user + "/pilot/images/deploy-ramdisk-ironic.tar"
        Scp.put_file(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                     self.settings.director_install_account_pwd, self.settings.deploy_ram_disk_image, remote_file)

        remote_file = "/home/" + self.settings.director_install_account_user + "/pilot/images/discovery-ramdisk.tar"
        Scp.put_file(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                     self.settings.director_install_account_pwd, self.settings.discovery_ram_disk_image, remote_file)

        remote_file = "/home/" + self.settings.director_install_account_user + "/pilot/images/overcloud-full.tar"
        Scp.put_file(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                     self.settings.director_install_account_pwd, self.settings.overcloud_image, remote_file)
    ...
    def node_discovery(self):
        ...
        if self.settings.use_custom_instack_json is True:
            ...
            logger.debug(Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                                 self.settings.director_install_account_user,
                                                 self.settings.director_install_account_pwd, cmd))

            remote_file = "/home/" + self.settings.director_install_account_user + "/instackenv.json"
            Scp.put_file(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                         self.settings.director_install_account_pwd, self.settings.custom_instack_json, remote_file)
        else:
            cmds = [
            ...
            # where nic's dont get detected .. resetting idrac & powering on the node seems to do it.
            for node in (self.settings.controller_nodes + self.settings.compute_nodes + self.settings.ceph_nodes):
                while 1:
                    cmd = ". $HOME/.bash_profile;idracula -u " + self.settings.ipmi_user \
                          + " -p '" + self.settings.ipmi_password + "' -scan '" \
                          + node.idrac_ip + "-" + node.idrac_ip + "'"
                    out = Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                                  self.settings.director_install_account_user,
                                                  self.settings.director_install_account_pwd, cmd)[0]
                    if "No integrated 1 GB nics" in out or "No WSMAN endpoint at" in out:
                        ...
                        if "No integrated 1 GB nics" in out:
                            ...
                            cmd = "cd ~/pilot/probe_idrac/probe_idrac/;./probe_idrac.py -l " + self.settings.ipmi_user \
                                  + " -p " + self.settings.ipmi_password + " " \
                                  + node.idrac_ip + " > " + node.idrac_ip + ".dump"
                            Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                                    self.settings.director_install_account_user,
                                                    self.settings.director_install_account_pwd, cmd)

                        logger.debug("reseting idrac")
                        ipmi_session = Ipmi(self.settings.cygwin_installdir, self.settings.ipmi_user,
            ...
            cmd = ". $HOME/.bash_profile;idracula -u " + self.settings.ipmi_user + " -p '" + self.settings.ipmi_password \
                  + "' -scan '" + self.settings.ipmi_discovery_range_start + "-" \
                  + self.settings.ipmi_discovery_range_end + "' > ~/instackenv.json"
            logger.debug(Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                                 self.settings.director_install_account_user,
                                                 self.settings.director_install_account_pwd, cmd))

            cmd = "ls -la ~/instackenv.json | awk '{print $5;}'"
            size = Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                           self.settings.director_install_account_user,
                                           self.settings.director_install_account_pwd, cmd)[0]
            if int(size) <= 50:
                logger.fatal("did not manage to pick up the nodes..")
                raise AssertionError(
                    "Unable to scan all the nodes ... need to go & pull the plug(s) - " + size + " - " + size[0])

            else:
                logger.debug("nodes appear to have been picked up")

        for node in (self.settings.controller_nodes + self.settings.compute_nodes + self.settings.ceph_nodes):
            ipmi_session = Ipmi(self.settings.cygwin_installdir, self.settings.ipmi_user, self.settings.ipmi_password,
                                node.idrac_ip)
            ipmi_session.power_off()
            time.sleep(60)
    ...
    def assign_node_roles(self):
        ...
        for node in self.settings.controller_nodes:
            cmd = 'cd ' + "/home/" + self.settings.director_install_account_user \
                  + ";source stackrc;cd ~/pilot;./assign_role.py " + node.provisioning_mac_address + " controller"
            out = Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                          self.settings.director_install_account_user,
                                          self.settings.director_install_account_pwd, cmd)
        ...
        for node in self.settings.compute_nodes:
            cmd = 'cd ' + "/home/" + self.settings.director_install_account_user \
                  + ";source stackrc;cd ~/pilot;./assign_role.py " + node.provisioning_mac_address + " compute"
            out = Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                          self.settings.director_install_account_user,
                                          self.settings.director_install_account_pwd, cmd)
            if "Not Found" in out[0]:
        ...
        for node in self.settings.ceph_nodes:
            cmd = 'cd ' + "/home/" + self.settings.director_install_account_user \
                  + ";source stackrc;cd ~/pilot;./assign_role.py " + node.provisioning_mac_address + " storage"
            out = Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                          self.settings.director_install_account_user,
                                          self.settings.director_install_account_pwd, cmd)
            if "Not Found" in out[0]:
    ...
    def setup_storage(self):
        ...
        ceph_yaml = os.path.join("pilot", "templates", "overrides", "puppet", "hieradata", "ceph.yaml")
        src_name = os.path.join(self.settings.foreman_configuration_scripts, ceph_yaml)
        ...
        for line in src_file:
            if line.startswith(osds_param):
                ...
                if len(tokens) > 0 and (tokens[0].startswith("#")
                   or tokens[0].startswith("'") or tokens[0].startswith("journal:")):
                    continue
        ...
        dst_name = os.path.join(os.path.join("/home", install_admin_user, ceph_yaml))
        Scp.put_file(self.settings.director_node.external_ip, install_admin_user, install_admin_password, tmp_name,
                     dst_name)
        os.remove(tmp_name)
    ...
    def setup_eqlx(self):
        ...
        Scp.put_file(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                     self.settings.director_install_account_pwd, self.settings.eqlx_yaml, eqlx_yaml)
        ...
        cmds = [
            ...
            'sed -i "s|CinderEqlxSanThinProvision: .*|CinderEqlxSanThinProvision: '
        ...
        cmd = "sudo sed -i 's|2015-11-06|2015-10-15|' /home/" \
              + self.settings.director_install_account_user \
              + "/pilot/templates/overcloud/puppet/extraconfig/pre_deploy/controller/cinder-eqlx.yaml"
        Ssh.execute_command_tty(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                                self.settings.director_install_account_pwd, cmd)
        cmd = "sudo sed -i 's|2015-11-06|2015-10-15|' " \
              "/usr/share/openstack-tripleo-heat-templates/puppet/extraconfig/pre_deploy/controller/cinder-eqlx.yaml"
        Ssh.execute_command_tty(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                                self.settings.director_install_account_pwd, cmd)
    ...
    def setup_dellsc(self):
        ...
        #  leave the undercloud intact but want to redeploy with a different config
        dellsc_yaml = "/home/" + self.settings.director_install_account_user \
                      + "/pilot/templates/dell-dellsc-environment.yaml"
        Scp.put_file(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                     self.settings.director_install_account_pwd, self.settings.dellsc_yaml, dellsc_yaml)

        cmds = [
            'sed -i "s|CinderEnableDellScBackend: .*|CinderEnableDellScBackend: true|" '
            + dellsc_yaml,
            'sed -i "s|CinderDellScBackendName: .*|CinderDellScBackendName: '
            + "'" + self.settings.dellsc_backend_name + "'" + '|" ' + dellsc_yaml,
            ...
            'sed -i "s|CinderDellScIscsiIpAddress: .*|CinderDellScIscsiIpAddress: '
            + self.settings.dellsc_iscsi_ip_address + '|" ' + dellsc_yaml,
            'sed -i "s|CinderDellScIscsiPort: .*|CinderDellScIscsiPort: '
            + "'" + self.settings.dellsc_iscsi_port + "'" + '|" ' + dellsc_yaml,
            'sed -i "s|CinderDellScApiPort: .*|CinderDellScApiPort: '
            + "'" + self.settings.dellsc_api_port + "'" + '|" ' + dellsc_yaml,
            'sed -i "s|CinderDellScServerFolder: .*|CinderDellScServerFolder: '
            + "'" + self.settings.dellsc_server_folder + "'" + '|" ' + dellsc_yaml,
            'sed -i "s|CinderDellScVolumeFolder: .*|CinderDellScVolumeFolder: '
            + "'" + self.settings.dellsc_volume_folder + "'" + '|" ' + dellsc_yaml,
        ...
        cmd = "sudo sed -i 's|2015-11-06|2015-10-15|' /usr/share/openstack-" \
              "tripleo-heat-templates/puppet/extraconfig/pre_deploy/controller/cinder-dellsc.yaml"
        Ssh.execute_command_tty(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                                self.settings.director_install_account_pwd, cmd)
    ...
    def setup_networking(self):
        ...
        network_yaml = "/home/" + install_admin_user + "/pilot/templates/network-environment.yaml"
        storage_yaml = "/home/" + install_admin_user + "/pilot/templates/nic-configs/ceph-storage.yaml"
        compute_yaml = "/home/" + install_admin_user + "/pilot/templates/nic-configs/compute.yaml"
        controller_yaml = "/home/" + install_admin_user + "/pilot/templates/nic-configs/controller.yaml"

        # Re - Upload the yaml files in case we're trying to
        # leave the undercloud intact but want to redeploy with a different config
        Scp.put_file(self.settings.director_node.external_ip, install_admin_user, install_admin_password,
                     self.settings.network_env_yaml, network_yaml)
        Scp.put_file(self.settings.director_node.external_ip, install_admin_user, install_admin_password,
                     self.settings.ceph_storage_yaml, storage_yaml)
        Scp.put_file(self.settings.director_node.external_ip, install_admin_user, install_admin_password,
                     self.settings.compute_yaml, compute_yaml)
        Scp.put_file(self.settings.director_node.external_ip, install_admin_user, install_admin_password,
                     self.settings.controller_yaml, controller_yaml)

        # cmd = "sudo chmod 777 " +network_yaml
        # logger.debug( Ssh.execute_command(self.settings.director_node.external_ip,
        # self.settings.director_install_account_user, self.settings.director_install_account_pwd,cmd))

        cmds = [
            'sed -i "s|ControlPlaneDefaultRoute:.*|ControlPlaneDefaultRoute: '
            ...
            'sed -i "s|InternalApiAllocationPools:.*|InternalApiAllocationPools: '
            + "[{'start': '" + self.settings.private_api_allocation_pool_start
            + "', 'end': '" + self.settings.private_api_allocation_pool_end + "'}]"   '|" ' + network_yaml,
            'sed -i "s|StorageAllocationPools:.*|StorageAllocationPools: '
            + "[{'start': '" + self.settings.storage_allocation_pool_start
            + "', 'end': '" + self.settings.storage_allocation_pool_end + "'}]"   '|" ' + network_yaml,
            'sed -i "s|StorageMgmtAllocationPools:.*|StorageMgmtAllocationPools: '
            + "[{'start': '" + self.settings.storage_cluster_allocation_pool_start
            + "', 'end': '" + self.settings.storage_cluster_allocation_pool_end + "'}]"   '|" ' + network_yaml,
            'sed -i "s|ExternalAllocationPools:.*|ExternalAllocationPools: '
            + "[{'start': '" + self.settings.external_allocation_pool_start
            + "', 'end': '" + self.settings.external_allocation_pool_end + "'}]"   '|" ' + network_yaml,
            'sed -i "s|ExternalInterfaceDefaultRoute:.*|ExternalInterfaceDefaultRoute: '
            ...
            'sed -i "s|ProvisioningNetworkGateway:.*|ProvisioningNetworkGateway: '
            + self.settings.provisioning_gateway + '|" ' + network_yaml,
            'sed -i "s|ControlPlaneDefaultRoute:.*|ControlPlaneDefaultRoute: '
            + self.settings.director_node.provisioning_ip + '|" ' + network_yaml,
            "sed -i 's|ControlPlaneSubnetCidr:.*|ControlPlaneSubnetCidr: " + '"' +
            self.settings.provisioning_network.split("/")[1] + '"' + "|' " + network_yaml,
            'sed -i "s|EC2MetadataIp:.*|EC2MetadataIp: ' +
            self.settings.director_node.provisioning_ip + '|" ' + network_yaml,
            "sed -i 's|DnsServers:.*|DnsServers: " + '["'
            + self.settings.name_server + '"]|' + "' " + network_yaml,
            'sed -i "s|InternalApiNetworkVlanID:.*|InternalApiNetworkVlanID: '
            ...
            'sed -i "s|StorageMgmtNetworkVlanID:.*|StorageMgmtNetworkVlanID: '
        ...
        if self.settings.controller_bond_opts == self.settings.compute_bond_opts \
                and self.settings.compute_bond_opts == self.settings.storage_bond_opts:
            logger.debug(
                "applying " + self.settings.settings.compute_bond_opts
                + " bond mode to all the nodes (network-environment.yaml)")
            cmds = [
                'sed -i "s|      \\"mode=802.3ad miimon=100\\"|      \\"mode='
            ...
            cmds = [
                'sed -i "s|em1|' + self.settings.controller_bond0_interfaces.split(" ")[0] + '|" ' + controller_yaml,
                'sed -i "s|p3p1|' + self.settings.controller_bond0_interfaces.split(" ")[1] + '|" ' + controller_yaml,
                'sed -i "s|em2|' + self.settings.controller_bond1_interfaces.split(" ")[0] + '|" ' + controller_yaml,
                'sed -i "s|p3p2|' + self.settings.controller_bond1_interfaces.split(" ")[1] + '|" ' + controller_yaml,
                'sed -i "s|em3|' + self.settings.controller_provisioning_interface + '|" ' + controller_yaml,
                'sed -i "s|192.168.110.0/24|' + self.settings.management_network + '|" ' + controller_yaml,
                'sed -i "s|192.168.120.1|' + self.settings.provisioning_gateway + '|" ' + controller_yaml,
            ...
            cmds = ['sed -i "s|em1|' + self.settings.compute_bond0_interfaces.split(" ")[0] + '|" ' + compute_yaml,
                    'sed -i "s|p3p1|' + self.settings.compute_bond0_interfaces.split(" ")[1] + '|" ' + compute_yaml,
                    'sed -i "s|em2|' + self.settings.compute_bond1_interfaces.split(" ")[0] + '|" ' + compute_yaml,
                    'sed -i "s|p3p2|' + self.settings.compute_bond1_interfaces.split(" ")[1] + '|" ' + compute_yaml,
                    'sed -i "s|em3|' + self.settings.compute_provisioning_interface + '|" ' + compute_yaml
                    ]
            for cmd in cmds:
                logger.debug(Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                                     self.settings.director_install_account_user,
            ...
            cmds = ['sed -i "s|em1|' + self.settings.storage_bond0_interfaces.split(" ")[0] + '|" ' + storage_yaml,
                    'sed -i "s|p2p1|' + self.settings.storage_bond0_interfaces.split(" ")[1] + '|" ' + storage_yaml,
                    'sed -i "s|em2|' + self.settings.storage_bond1_interfaces.split(" ")[0] + '|" ' + storage_yaml,
                    'sed -i "s|p2p2|' + self.settings.storage_bond1_interfaces.split(" ")[1] + '|" ' + storage_yaml,
                    'sed -i "s|em3|' + self.settings.storage_provisioning_interface + '|" ' + storage_yaml
                    ]
            for cmd in cmds:
                logger.debug(Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                                     self.settings.director_install_account_user,
    ...
    def deploy_overcloud(self):
        ...
        cmd = "cd ~/pilot;source ~/stackrc;./deploy-overcloud.py" + " --computes " \
              + str(len(self.settings.compute_nodes)) + " --controllers " \
              + str(len(self.settings.controller_nodes)) + " --storage " \
              + str(len(self.settings.ceph_nodes)) + " --vlan " \
              + self.settings.tenant_vlan_range + " > overcloud_deploy_out.log"
    ...
    def delete_overcloud(self):
        ...
        while 1:
            ...
            if "overcloud" in Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                                      self.settings.director_install_account_user,
                                                      self.settings.director_install_account_pwd, cmd)[0]:
                time.sleep(60)
            else:
        ...
        re = Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                     self.settings.director_install_account_user,
                                     self.settings.director_install_account_pwd, cmd)
        ls_nodes = re[0].split("\n")
        ls_nodes.pop()
    ...
    def retreive_nodes_ips(self):
        ...
        try:
            ...
            for each in ls_nodes:
                ...
                cmd = "ssh-keyscan {} >> ~/.ssh/known_hosts".format(
                    provisioning_ip)
    ...
    def run_sanity_test(self):
        if self.settings.run_sanity is True:
            ...
            remote_file = "/home/" + self.settings.director_install_account_user + "/sanity_test.sh"
            Scp.put_file(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                         self.settings.director_install_account_pwd, self.settings.sanity_test, remote_file)
    ...
    def run_tempest(self):
        ...
        cmds = [
            ...
            'source ~/overcloudrc;cd ~/tempest;tools/config_tempest.py --deployer-input '
        ...
        Ssh.execute_command_tty(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                                self.settings.director_install_account_pwd, cmd)
        Scp.get_file(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                     self.settings.director_install_account_pwd, "/auto_results/tempest.xml",
                     "/home/" + self.settings.director_install_account_user + "/tempest/tempest.xml")
        Scp.get_file(self.settings.director_node.external_ip, self.settings.director_install_account_user,
                     self.settings.director_install_account_pwd, "/auto_results/tempest.log",
                     "/home/" + self.settings.director_install_account_user + "/tempest/tempest.log")
    ...
    def fix_controllers_vlan_range(self):
        ...
        for each in controllers:
            ...
            cmd = "ssh heat-admin@" + provisioning_ip \
                  + " \"sudo sed -i 's/^network_vlan_ranges.*/network_vlan_ranges=physint:" \
                  + self.settings.tenant_vlan_range + ",physext/' /etc/neutron/plugin.ini\""
            logger.debug(Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                                 self.settings.director_install_account_user,
                                                 self.settings.director_install_account_pwd, cmd))
    ...
    def fix_cinder_conf(self):
        ...
        for each in controllers:
            provisioning_ip = each.split("|")[6].split("=")[1]
            cmd = "ssh heat-admin@" + provisioning_ip + " /sbin/ifconfig | grep \"inet.*\." \
                  + self.settings.private_api_vlanid + "\..*netmask " \
                  + self.settings.private_api_netmask + ".*\" | awk '{print $2}'"
            re = Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                         self.settings.director_install_account_user,
                                         self.settings.director_install_account_pwd, cmd)
            private_api = re[0].split("\n")[0].rstrip()
            ...
            cmds = [
                " ssh heat-admin@" + provisioning_ip
                + " \"sudo sh -c 'echo \\\"[keymgr]\\\" >> /etc/cinder/cinder.conf'\"",
                " ssh heat-admin@" + provisioning_ip
                + " \"sudo sh -c 'echo \\\"encryption_auth_url=http://"
                + private_api + ":5000/v3\\\" >> /etc/cinder/cinder.conf'\""
            ]
            for cmd in cmds:
                logger.debug(Ssh.execute_command_tty(self.settings.director_node.external_ip,
                                                     self.settings.director_install_account_user,
    ...
    def configure_calamari(self):
        ...
        cmd = 'source stackrc;cd ~' + self.settings.director_install_account_user \
              + '/pilot;./config_calamari_nodes.sh ' + self.settings.ceph_node.external_ip \
