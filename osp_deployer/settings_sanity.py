...
class DeployerSanity():
    ...
    def check_files(self):
        ...
        shouldbbevalidips = [
            ...
            'private_api_vlanid', 'private_api_netmask', 'private_api_allocation_pool_start',
        ...
        for ip in getattr(self.settings, 'discovery_ip_range').split(","):
            self.is_valid_ip(ip), "Setting for discovery_ip_range " + ip + " is not a valid ip "
        for each in shouldbbevalidips:
            assert self.is_valid_ip(
                getattr(self.settings, each)), "Setting for " + each + " is not a valid ip " + getattr(self.settings,
                                                                                                       each)

        assert os.path.isfile(self.settings.rhl72_iso), self.settings.rhl722_iso + "ISO doesn't seem to exist"
        assert os.path.isfile(
            self.settings.sah_kickstart), self.settings.sah_kickstart + "kickstart file doesnn't seem to exist"
        assert os.path.isfile(
            self.settings.director_deploy_sh), self.settings.director_deploy_sh + " script doesn't seem to exist"
        assert os.path.isfile(
            self.settings.undercloud_conf), self.settings.undercloud_conf + " file doesn't seem to exist"
        if self.settings.use_custom_instack_json is True:
            assert os.path.isfile(
                self.settings.custom_instack_json), self.settings.custom_instack_json + " file doesn't seem to exist"

        assert os.path.isfile(
            self.settings.deploy_ram_disk_image), self.settings.deploy_ram_disk_image + " file doesn't seem to exist"
        assert os.path.isfile(
            self.settings.discovery_ram_disk_image), self.settings.discovery_ram_disk_image \
            + " file doesn't seem to exist"
        assert os.path.isfile(
            self.settings.overcloud_image), self.settings.overcloud_image + " file doesn't seem to exist"
        assert os.path.isfile(
            self.settings.install_director_sh), self.settings.install_director_sh + " file doesn't seem to exist"
    ...
    def check_network_settings(self):
        ...
        shouldhaveattributes = ['hostname', 'idrac_ip', 'root_password', 'anaconda_ip', 'anaconda_iface',
        ...
        for each in shouldhaveattributes:
            assert hasattr(self.settings.sah_node,
                           each), self.settings.network_conf + " SAH node has no " + each + " attribute"

        shouldbbevalidips = ['idrac_ip', 'anaconda_ip', 'external_ip',
                             'provisioning_ip', 'storage_ip', 'public_api_ip', 'private_api_ip', 'managment_ip']
        for each in shouldbbevalidips:
            assert self.is_valid_ip(getattr(self.settings.sah_node, each)), "SAH node " + each + " is not a valid ip"

        # Verify director network definition
        logger.debug("verifying director vm network settings")
        shouldhaveattributes = ['hostname', 'root_password', 'external_ip', 'provisioning_ip',
                                'managment_ip', 'public_api_ip', 'private_api_ip']
        for each in shouldhaveattributes:
            assert hasattr(self.settings.director_node,
                           each), self.settings.network_conf + " director node has no " + each + " attribute"
            shouldbbevalidips = ['external_ip', 'provisioning_ip', 'managment_ip', 'public_api_ip', 'private_api_ip']
        for each in shouldbbevalidips:
            assert self.is_valid_ip(
                getattr(self.settings.director_node, each)), "director_node node " + each + " is not a valid ip"

        # Verify Ceph vm node network definition
        logger.debug("verifying ceph vm network settings")
        shouldhaveattributes = ['hostname', 'root_password', 'external_ip', 'storage_ip']
        for each in shouldhaveattributes:
            assert hasattr(self.settings.ceph_node,
                           each), self.settings.network_conf + " Ceph Vm node has no " + each + " attribute"
        ...
        for controller in self.settings.controller_nodes:
            shouldhaveattributes = ['hostname', 'idrac_ip', 'provisioning_mac_address']
            for each in shouldhaveattributes:
                assert hasattr(controller, each), controller.hostname + " node has no " + each + " attribute"
                shouldbbevalidips = ['idrac_ip']
            for each in shouldbbevalidips:
                assert self.is_valid_ip(
                    getattr(controller, each)), controller.hostname + " node " + each + " is not a valid ip"
        ...
        for compute in self.settings.compute_nodes:
            ...
            for each in shouldbbevalidips:
                assert self.is_valid_ip(
                    getattr(compute, each)), compute.hostname + " node " + each + " is not a valid ip"
        ...
        for storage in self.settings.ceph_nodes:
            ...
            for each in shouldbbevalidips:
                assert self.is_valid_ip(
                    getattr(storage, each)), storage.hostname + " node " + each + " is not a valid ip"
